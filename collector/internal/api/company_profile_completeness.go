...
// 채웠는데 게이트에 막힌다"는 부당한 상황이 생겼다(사용자 확인). 그래서
// 지역/업종/기업규모(온보딩에서 건너뛸 수 없는 필수 3개) 각각에 20%(합
// 60%)를, 나머지 9개 선택 카테고리가 남은 40%를 균등분배(각 40/9%)받도록
// 가중평균으로 바꿨다. 이 재설계에 맞춰 "업종"의 기존 부분점수 공식
// (선택한 업종 수/10)도 이진(하나라도 선택하면 100%)으로 바꿨다 — 안
// 그러면 업종을 1개만 선택한 흔한 경우(대부분의 중소기업) industry가
// 10%만 인정돼 필수 3개를 다 채워도 60%에 못미치는 모순이 생긴다.
// ⚠️같은 날 3차 조정 — onboardingMinCompleteness를 50→70으로 올리면서
// "필수 3개만 채우면 게이트를 항상 넘는다"는 처음 설계 의도는 깨졌다
// (60% < 70%). 가중치 공식 자체는 그대로 두고 문턱만 올린 것 — 필수
// 3개 외에 선택 카테고리를 최소 하나 이상 채워야 온보딩을 끝낼 수 있다.
package api
...
// 각각의 가중치(%). 3개 합 60%로 onboardingMinCompleteness(70%)에는
// 못 미친다(선택 카테고리를 최소 하나 더 채워야 함, 의도된 정책). index.html의
...
// onboardingMinCompleteness — 2026-08-05 온보딩 재설계에서 50으로
// 확정했다가, 같은 날 70으로 상향 조정(사용자 확정). 프론트도 같은
// 기준(70)으로 완료 버튼을 잠그지만, 그건 UX일 뿐 신뢰 경계가 아니다 —
// 실제 게이트는 여기, 서버가 최신 데이터로 다시 계산해서 확인한다.
// ⚠️필수 3개(지역/업종/기업규모)만 채우면 60%인데(requiredCompletenessCategoryWeight
// 참고) 70% 문턱은 그보다 높다 — 즉 이제 필수 3개만으로는 온보딩을
// 끝낼 수 없고, 선택 카테고리를 최소 하나 이상 더 채워야 한다(의도된
// 정책 변경, "나중에 하기" 버튼 폐지와 함께 온보딩을 더 꼼꼼히 받도록
// 강화).
const onboardingMinCompleteness = 70
