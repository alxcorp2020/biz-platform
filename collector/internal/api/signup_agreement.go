// signup_agreement.go — 회원가입 2단계(renderSignupProfileStep, 이메일
// 가입자와 소셜 로그인 가입자 공통 화면)에서 연락처 등록 + 필수 약관
// 동의를 처리한다. PUT /api/me/company-profile(handleUpsertCompanyProfile)
// 과 일부러 분리했다 — 그 엔드포인트는 가입 2단계뿐 아니라 이후 "회사
// 정보 수정" 화면에서도 재사용되는데, 거기에 연락처/약관동의를 필수로
// 묶으면 기존 회원이 프로필을 고칠 때마다 약관에 매번 동의해야 하는
// 이상한 흐름이 된다.
package api
...
// 금액 검증과 같은 원칙 — billing.go 참고). 연락처는 형식 검증 후
// users.phone_number에 저장.
